...
function addEvidenceCard(data, timestamp = null) {
    ...
    // V7.3: Individual Discovery - Each detection gets a unique ID
    // We iterate through detections to ensure every hit gets a card
    detections.forEach((det, idx) => {
        const label = det.label.toUpperCase();
        const baseId = `evidence-${ts.toFixed(3).replace('.', '-')}-${label}-${idx}`;
        ...
        if (!card) {
            ...
            const crop = isInsight ? data.thumbnail : (det.crop || null);
            ...
            card.innerHTML = `
                ...
                <div class="evidence-info">
                    <div class="evidence-type">ENTRY_${ts.toFixed(2)}</div>
                    <div class="evidence-time">TAGS: ${label}</div>
